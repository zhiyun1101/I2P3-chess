...
int submission(State* root, int depth, int alpha, int beta, int op, int selfplayer)
{
    ...
    if(op == MAXimize)
    {
        ...
        {   
            value = std::max(value, submission(root->next_state(move), depth-1, alpha, beta, MINimize, selfplayer));
    ...
    else //op == MINimize
    {
        ...
        for(auto move: root->legal_actions)
        {
            value = std::min(value, submission(root->next_state(move), depth-1, alpha, beta, MAXimize, selfplayer));
...
Move Submission::get_move(State* state, int depth)
{
    ...
    {   
        int childvalue = submission(state->next_state(move), depth-1, -INT_MAX, INT_MAX, MINimize, state->player);
